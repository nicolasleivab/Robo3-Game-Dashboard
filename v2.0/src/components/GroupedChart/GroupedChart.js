...
class GroupedChart extends Component {
    ...
    fetch(
      ...
      .then(
        result => {
          ...
          const students = sheetsData.map(function(d) {
            //create new array with students
            return d.ID;
          });
          const allStudents = students.filter(function(item, pos) {
            //get unique values
            return students.indexOf(item) === pos;
          });
          //get default person code
          const personFilter = this.state.personFilter;
          const filteredData = sheetsData.filter(
            word => word.ID === personFilter
          );
  ...
  filterByStudent = () => {
    const currentStudent = document.getElementById("student-autocomplete")
      .value;
